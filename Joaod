-- RAT HUB - VERSÃO ULTRA PREMIUM (TP BONITO)
-- by Jao
...
somWhoosh.SoundId = "rbxassetid://91203790950646"
somWhoosh.Volume = 0.15
somWhoosh.Parent = player
...
-- ==================== EFEITOS DE TP ====================
-- 1. FLASH BRANCO
local function flashBranco()
    local flash = Instance.new("Frame", gui)
    flash.Size = UDim2.new(1, 0, 1, 0)
    flash.BackgroundColor3 = corBranco
    flash.BackgroundTransparency = 1
    flash.ZIndex = 999
    flash.Parent = gui
    
    ts:Create(flash, TweenInfo.new(0.1), {BackgroundTransparency = 0.3}):Play()
...
-- 5. CÍRCULO MÁGICO
local function criarCirculoMagico(pos)
    for i = 1, 12 do
        local angle = (i / 12) * math.pi * 2
        local p = Instance.new("Part")
        p.Size = Vector3.new(0.2, 0.1, 0.2)
        p.Position = pos + Vector3.new(math.cos(angle) * 2.5, 0.1, math.sin(angle) * 2.5)
        p.Anchored = true
        p.CanCollide = false
        p.Material = Enum.Material.Neon
        p.Color = corRoxo
...
-- 7. PARTÍCULAS EM ESPIRAL
local function criarEspiral(pos)
    for i = 1, 20 do
        local angle = (i / 20) * math.pi * 4
        local radius = 1 + (i / 20) * 3
        local p = Instance.new("Part")
        p.Size = Vector3.new(0.2, 0.2, 0.2)
        p.Position = pos + Vector3.new(math.cos(angle) * radius, (i / 20) * 2, math.sin(angle) * radius)
        p.Anchored = true
        p.CanCollide = false
        p.Material = Enum.Material.Neon
        p.Color = corRoxo
...
-- 8. ONDA DE LUZ
local function criarOnda(pos)
    for i = 1, 10 do
        local p = Instance.new("Part")
        p.Size = Vector3.new(i * 0.5, 0.1, i * 0.5)
        p.Position = pos
        p.Anchored = true
        p.CanCollide = false
        p.Material = Enum.Material.Neon
...
-- 9. EFEITO DE POEIRA
local function criarPoeira(pos)
    for i = 1, 15 do
        local p = Instance.new("Part")
        p.Size = Vector3.new(0.1, 0.1, 0.1)
        p.Position = pos + Vector3.new(math.random(-3,3), math.random(0,2), math.random(-3,3))
        p.Anchored = true
        p.CanCollide = false
        p.Material = Enum.Material.Neon
        p.Color = corRoxoClaro
...
fundoGrad.Color = ColorSequence.new({
    ...
    ColorSequenceKeypoint.new(0.3, Color3.fromRGB(20, 5, 40)),
    ColorSequenceKeypoint.new(0.6, Color3.fromRGB(30, 10, 50)),
...
minimizarBtn.MouseButton1Click:Connect(function()
    ...
    if minimizado then
        ts:Create(main, TweenInfo.new(0.2), {Size = UDim2.new(0, 180, 0, 45)}):Play()
    else
        ts:Create(main, TweenInfo.new(0.2), {Size = UDim2.new(0, 180, 0, 250)}):Play()
    end
end)
